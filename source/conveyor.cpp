...
void purge_debris(ChSystem& mysystem, double max_age = 5.0)
{
	...
	{
		std::shared_ptr<ChBody> abody = (*mysystem.Get_bodylist())[i];
		...
		for (unsigned int na= 0; na< abody->GetAssets().size(); na++)
		{
			...
			{
				std::shared_ptr<ElectricParticleProperty> electricproperties = myasset;
		...
		if (to_delete)
		{
			...
			std::shared_ptr<ChBody> mysharedbody(abody); // ..shared pointer
